...
import { getReadableStream } from '../../../api/storage/storage.service';
import { escapeCsvValue } from "../../../utils/stringManipulation";
import { createPool } from "../../supabase.service";
import { mapVaersDataRow, mapVaersVaccineRow } from "../vaers.maps";
import { batchCreateVaersSymptomRow, createVaersReportRow, getVaersReportRowByVaersId, getVaersSymptomRowBySymptomAndVaersId, updateVaersReportRow } from "../vaers.repository";
import { VaersDataRawRow, vaersReportColumnsString, VaersReportRowInsert, VaersSymptomRaw, VaersSymptomRowInsert, VaersTables, vaersVaccineColumnsString, VaersVaccineRawRow } from "../vaers.types";
...
function mapSymptomRows(row: VaersSymptomRaw, path: string): VaersSymptomRowInsert[] {
...
export async function copyParsedSymptomCsvToDb(filePath: string) {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            const copyStream = client.query(copyFrom(`COPY ${VaersTables.SYMPTOM} (vaers_id, symptom, symptom_version, csv_url) FROM STDIN WITH CSV`));
...
export async function copyParsedVaxDataCsvToDb(filePath: string) {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            const copyStream = client.query(copyFrom(`COPY ${VaersTables.REPORT} (${vaersReportColumnsString}) FROM STDIN WITH CSV`));
            ...
            const transformer = new Transform({
                ...
                transform(chunk, encoding, callback) {
                    ...
                    const csvLine = Object.values(mappedRow)
...
export async function copyParsedVaccineCsvToDb(filePath: string) {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            const copyStream = client.query(copyFrom(`COPY ${VaersTables.VACCINE} (${vaersVaccineColumnsString}) FROM STDIN WITH CSV`));
...
async function handleSymptomRow(row: VaersSymptomRaw, path: string): Promise<VaersSymptomRowInsert[]> {
    ...
    for (const newRow of mappedRows) {
        ...
        try {
            existingRow = await getVaersSymptomRowBySymptomAndVaersId(newRow.vaers_id, newRow.symptom);
        } catch (error) { }
...
async function handleVaxRow(row: VaersVaccineRawRow | VaersDataRawRow, path: string): Promise<VaersReportRowInsert | null> {
    try {
        ...
        if (existingRow) {
            ...
            return await createVaersReportRow(mappedRow);
        }
    } catch (error) {
...
export const rowHandler = async (row: VaersDataRawRow | VaersVaccineRawRow | VaersSymptomRaw, path: string) => {
