import {
  createVaersSupabaseClient,
  pgpVaersDbConnection,
...
export const batchCreateVaersReportRow = async (
  values: VaersReportRowInsert[]
): Promise<VaersReportRow[] | null> => {
  try {
    const { data, error } = await supabase
      .from(SupabaseTables.rawReports)
      .upsert(values);
    if (error) {
...
export const getVaersReportRowByVaersId = async (
  vaersId: number
): Promise<VaersReportRow | null> => {
  try {
    const { data, error } = await supabase
...
export const updateVaersReportRow = async (
  id: string,
  values: VaersReportRowInsert
): Promise<VaersReportRow> => {
  try {
    const { data, error } = await supabase
      .from(SupabaseTables.rawReports)
      .update(values)
...
export const createVaersSymptomRow = async (
  values: VaersSymptomRowInsert
): Promise<VaersSymptomRow | null> => {
  try {
    const { data, error } = await supabase
      .from(SupabaseTables.rawSymptoms)
      .insert({ ...values });
...
export const batchCreateVaersSymptomRow = async (
  values: VaersSymptomRowInsert[]
): Promise<VaersSymptomRow[] | null> => {
  try {
    const { data, error } = await supabase
...
export const getSymptomsBySymptom = async (
  vaccine: string,
  symptom: string
) => {
  const { data, error } = await supabase
...
export const getVaersSymptomRowBySymptomAndVaersId = async (
  vaersId: number,
  symptom: string
): Promise<VaersSymptomRow | null> => {
  try {
    ...
    if (error) {
      console.log(
        "Error getting vaers symptom row by vaers id:",
        error,
        vaersId,
        symptom
      );
...
export const getVaccineTypeStatsWhereSymptomsAreZero = async (
  count: number
) => {
  const { data, error } = await supabase
    .from(SupabaseTables.uniqueVaccineTypes)
  ...
  if (error) {
    console.log(
      "Error getting vaccine type stats where symptoms are zero:",
      error
    );
    throw error;
  }
...
export const getSymptomTypeStatsWhereUpdatedNull = async (count: number) => {
  const { data, error } = await supabase
    .from(SupabaseTables.uniqueSymptomTypes)
    .select("*")
    .is("updated_at", null)
    .limit(count);
  if (error) {
...
export const runUpdateVaccineSymptomStats = async (symptom: string) => {
  let { data, error } = await supabase.rpc("update_vaccine_symptom_stats", {
